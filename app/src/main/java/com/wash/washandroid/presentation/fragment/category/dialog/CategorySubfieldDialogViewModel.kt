...
class CategorySubfieldDialogViewModel : ViewModel() {
        ...
        viewModelScope.launch {
            try {
                ...
                if (response.isSuccessful && response.body()?.isSuccess == true) {
                    ...
                    Log.e("CategorySubfieldDialogViewModel", "Failed to add category type: ${response.errorBody()?.string()}")
                }
            } catch (e: Exception) {
                // 에러 처리
                Log.e("CategorySubfieldDialogViewModel", "Error adding category type: ${e.message}")
            }
        }
    }
}